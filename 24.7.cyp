...
CREATE(SecurityR:Security{title:"Security Risks"})
...
CREATE
  (SecurityR)-[:RISK]->(MitM),
  (SecurityR)-[:RISK]->(Insider),
  (SecurityR)-[:RISK]->(Phishing),
  (SecurityR)-[:RISK]->(DDOS),
  (SecurityR)-[:RISK]->(Malware),
  (Phishing)-[:RISK]->(Department),
  (Insider)-[:RISK]->(Fraud),
  (Insider)-[:RISK]->(Contact),
  (Phishing)-[:RISK]->(Contact),
  (Phishing)-[:RISK]->(Social),
  (DDOS)-[:RISK]->(Website),
  (MitM)-[:RISK]->(Contact),
  (Malware)-[:RISK]->(Department)
...
CREATE
  ...
  (Audit)-[:MONITOR]->(Department),
  (Audit)-[:MONITOR]->(Fraud),
  (Audit)-[:MONITOR]->(Contact),
  (Encryption)-[:SECURE]->(Contact),
  (Encryption)-[:SECURE]->(Department),
  (ACL)-[:MONITOR]->(Department),
  (ACL)-[:MONITOR]->(Website)